  ...
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        blockquote: ({ children }) => (
          <blockquote className="my-4 rounded-3xl border-l-4 border-cyan-400/60 bg-slate-900/70 p-4 text-slate-200">{children}</blockquote>
        ),
        table: ({ children }) => <table className="mb-4 w-full overflow-hidden rounded-3xl border border-slate-700 bg-slate-950 text-sm">{children}</table>,
        ...
        code: ({ inline, className, children }: any) => {
          ...
          return !inline && match ? (
            <div className="mb-4 overflow-hidden rounded-3xl border border-slate-700 bg-slate-950 text-sm">
              <SyntaxHighlighter
                style={oneDark}
                language={match[1]}
                PreTag="div"
            ...
            <code className="rounded-xl border border-slate-700 bg-slate-950 px-2 py-1 font-mono text-sm text-slate-100">{children}</code>
          );
        },
        pre: ({ children }) => <pre className="mb-4 overflow-x-auto rounded-3xl bg-slate-950 p-4 text-sm text-slate-100">{children}</pre>,
